...
export const wikiLintTool = {
    ...
    handler: async (args) => {
        try {
            ...
            const issueLines = report.issues.map(i => `- [${i.severity.toUpperCase()}] ${i.type}: ${i.message}`);
            return {
                content: [{
                        type: 'text',
